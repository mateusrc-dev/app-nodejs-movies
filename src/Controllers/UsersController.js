...
const UserRepository = require("../repositories/UserRepository") //importando o arquivo que tem as funções com as requisições para o banco de dados
const UserCreateService = require("../services/UserCreateService") //importando o arquivo que tem a lógica de cadastro de usuário
...
class UsersController { //vamos usar class porque ela permite que dentro dela possamos criar várias funções e acessar várias funções depois
  ...
  */
  async create(request, response) { //é aqui onde é processado as informações, a requisição do usuário e a resposta da requisição, não em rotes - aqui é onde o usuário é criado
    ...
    const userRepository = new UserRepository() //instanciando o banco de dados
    const userCreateService = new UserCreateService(userRepository) //vamos passar o parâmetro para o constructor ao instanciar o UserCreateService
    await userCreateService.execute({ name, email, password }) //passando os parâmetros que a função execute precisa pra realiza as lógicas de cadastro de usuário
